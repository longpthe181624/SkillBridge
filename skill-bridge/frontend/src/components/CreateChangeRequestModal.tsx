...
export default function CreateChangeRequestModal({
  ...
  const defaultType = contractType === 'Fixed Price' ? 'Add Scope' : 'RESOURCE_CHANGE';
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.desiredStartDate) {
      ...
      if (startDate < today) {
        newErrors.desiredStartDate = 'Desired Start Date must be in the future';
      } else if (startDate > maxDate) {
        newErrors.desiredStartDate = 'Desired Start Date must be within 5 years from today';
      }
    }
    ...
    if (!formData.desiredEndDate) {
      ...
      if (endDate < today) {
        newErrors.desiredEndDate = 'Desired End Date must be in the future';
      } else if (endDate > maxDate) {
        newErrors.desiredEndDate = 'Desired End Date must be within 5 years from today';
      } else if (formData.desiredStartDate) {
        const startDate = new Date(formData.desiredStartDate);
    ...
    if (formData.attachments.length > 0) {
      formData.attachments.forEach((file, index) => {
        ...
        if (!isValidType) {
          newErrors[`attachment_${index}`] = `File ${file.name} must be an image or PDF file`;
        }
        // Validate file size
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: error instanceof Error ? error.message : t('client.contractDetail.createChangeRequest.error.submitFailed') });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDraft = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: error instanceof Error ? error.message : t('client.contractDetail.createChangeRequest.error.saveDraftFailed') });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (formData.attachments.length + selectedFiles.length > MAX_FILES) {
        newErrors.attachments = `You can upload maximum ${MAX_FILES} file`;
        setErrors(newErrors);
        return;
      }
      ...
      selectedFiles.forEach((file, index) => {
        ...
        const isValidType = file.type.startsWith('image/') || file.type === 'application/pdf';
        if (!isValidType) {
          newErrors[`attachment_${formData.attachments.length + index}`] = `File ${file.name} must be an image or PDF file`;
        } else if (file.size > MAX_FILE_SIZE) {
          const errorMessage = t('client.contractDetail.createChangeRequest.validation.fileSize');
          newErrors[`attachment_${formData.attachments.length + index}`] = errorMessage.replace('{fileName}', file.name);
        } else {
      ...
      if (Object.keys(newErrors).length > 0) {
        ...
        // Remove attachments error if it exists
        const { attachments, ...restErrors } = errors;
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files) {
      ...
      // Check if adding files would exceed max limit
      if (formData.attachments.length + droppedFiles.length > MAX_FILES) {
        newErrors.attachments = `You can upload maximum ${MAX_FILES} file`;
        setErrors(newErrors);
        return;
      }
          ...
          <div className="mb-6">
            ...
            >
              ...
              <p className="text-gray-600">
                {t('client.contractDetail.createChangeRequest.attachments.uploadText')} (Images or PDF, max 1 file, max 10MB)
              </p>
              <input
                ref={fileInputRef}
                type="file"
                ...
                {formData.attachments.map((file, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                    ...
                    {errors[`attachment_${index}`] && (
                      <p className="text-red-500 text-sm mr-2">{errors[`attachment_${index}`]}</p>
                    )}
                    <button
                      type="button"
