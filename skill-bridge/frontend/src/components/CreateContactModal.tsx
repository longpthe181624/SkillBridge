...
export default function CreateContactModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : t('client.createContact.error.submitFailed'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white/95 backdrop-blur-md rounded-lg shadow-2xl p-8 w-full max-w-xl relative border border-gray-200">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6">
            <Label htmlFor="title">
              {t('client.createContact.titleLabel')} <span className="text-red-500">*</span>
            </Label>
            <Input
              id="title"
              type="text"
