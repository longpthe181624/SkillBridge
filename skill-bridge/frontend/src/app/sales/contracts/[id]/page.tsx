...
const MAX_ATTACHMENT_FILES = 1;
  ...
  const normalizedStatus = contractDetail?.status
    ? contractDetail.status.toLowerCase().replace(/[\s_]+/g, '')
    : '';

  // Check if contract is editable (Draft status or Request for Change)
  const isEditable = normalizedStatus === 'draft' || normalizedStatus === 'requestforchange';
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => {
      ...
      const filesToAdd = getValidatedAttachmentFiles(files, current.length);
  ...
  const handleSubmit = async (action: 'save' | 'draft') => {
    ...
    if (formData.effectiveStart && formData.effectiveEnd && formData.effectiveEnd < formData.effectiveStart) {
      toast({
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            if (formData.effectiveEnd && newStartDate && formData.effectiveEnd < newStartDate) {
                              toast({
                                title: 'Validation Error',
                                description: 'Effective End date must be on or after Effective Start date. End date will be updated.',
                                variant: 'destructive',
                              });
                              setFormData(prev => ({ ...prev, effectiveStart: newStartDate, effectiveEnd: newStartDate }));
                            } else {
                              setFormData(prev => ({ ...prev, effectiveStart: newStartDate }));
                            }
                          }}
                          disabled={!isEditable}
                          min={(() => {
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            if (formData.effectiveStart && newEndDate && newEndDate < formData.effectiveStart) {
                              toast({
                                title: 'Validation Error',
                                description: 'Effective End date must be on or after Effective Start date',
                                variant: 'destructive',
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center mb-4">
                      ...
                      >
                        ...
                        <span className="text-gray-600">
                          {t('sales.msaDetail.attachments.upload')} (PDF, max 1 file)
                        </span>
                      </label>
                    </div>
                  )}
