...
export default function SalesContractListPage() {
  ...
  // Fetch contracts
  // Map UI status selections to backend filter values
  const mapStatusForApi = (status: string) => {
    const normalized = status.trim().toLowerCase();
    if (normalized === 'internal review' || normalized === 'client under review') {
      // Both are represented by Under_Review in the backend
      return 'Under Review';
    }
    return status;
  };
  ...
  const fetchContracts = useCallback(async () => {
    ...
    try {
      ...
      const filters: SalesContractListFilters = {
        ...
        status: statusFilter && statusFilter !== 'All' ? mapStatusForApi(statusFilter) : undefined,
      ...
      // After fetching, trim results to the exact selected status when filtering review states
      const filteredContracts =
        statusFilter === 'Internal Review'
          ? response.contracts.filter(c => c.status === 'Internal Review')
          : statusFilter === 'Client Under Review'
            ? response.contracts.filter(c => c.status === 'Client Under Review')
            : response.contracts;
  ...
  const getStatusBadgeColor = (status: string) => {
    ...
    switch (normalized) {
      ...
      case 'underreview': // show as Client Under Review
  ...
  const getCloseRequestBadgeColor = (status?: string | null) => {
    const normalized = (status || '').toLowerCase();
    if (normalized === 'pending') return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    if (normalized === 'clientapproved') return 'bg-green-100 text-green-800 border-green-200';
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={statusFilter} onValueChange={handleFilterChange}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Request for Change">{t('sales.contracts.filter.requestChange')}</SelectItem>
                  ...
                  <SelectItem value="Cancelled">{t('sales.contracts.filter.cancelled')}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex items-center gap-2">
              <Select value={typeFilter} onValueChange={handleTypeFilterChange}>
                <SelectTrigger className="w-[150px] bg-white">
                  <SelectValue placeholder={t('sales.contracts.filter.type') || 'Type'} />
                </SelectTrigger>
                <SelectContent>
                ...
                {loading ? (
                  ...
                  contracts.map((contract) => (
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {contract.type === 'SOW' && contract.closeRequestStatus && (
                        <Badge className={`${getCloseRequestBadgeColor(contract.closeRequestStatus)} border`}>
                          {getCloseRequestLabel(contract.closeRequestStatus)}
                        </Badge>
                      )}
                    </div>
                      </TableCell>
