...
import { login } from '@/services/salesAuthService';
...
export default function SalesLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to sales dashboard (or opportunities list)
      router.push('/sales/opportunities');
    } catch (error: any) {
      setErrors({ submit: error.message || t('sales.login.errors.loginFailed') });
    } finally {
      setLoading(false);
    }
  };
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-6">
                <Label htmlFor="email" className="text-sm text-gray-700 mb-2 block">
                  {t('sales.login.emailLabel')} <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="email"
                  type="email"
                  placeholder={emailFocused ? '' : 'sales@landbridge.co.jp'}
              ...
              <div className="mb-6">
                <Label htmlFor="password" className="text-sm text-gray-700 mb-2 block">
                  {t('sales.login.passwordLabel')} <span className="text-red-500">*</span>
                </Label>
                <div className="relative">
                  <Input
                    id="password"
