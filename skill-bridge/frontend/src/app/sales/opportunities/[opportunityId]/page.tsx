...
export default function SalesOpportunityDetailPage() {
  ...
  const handleFileClick = async (e: React.MouseEvent<HTMLAnchorElement>, attachment: ProposalAttachment | string) => {
    e.preventDefault();
    if (!token) {
      toast({
        variant: "destructive",
    ...
    // Extract s3Key from attachment (could be object or string for backward compatibility)
    const s3Key = typeof attachment === 'string' ? attachment : attachment.s3Key;
    ...
    try {
      ...
      if (!s3Key) {
        toast({
          variant: "destructive",
          title: t('sales.opportunityDetail.messages.error'),
          description: 'Invalid file attachment',
        });
        return;
      }
                    ...
                    <div className="space-y-1">
                      {opportunity.proposal.attachments.map((attachment, index) => {
                        ...
                        if (typeof attachment === 'string') {
                          fileName = attachment.split('/').pop() || `${t('sales.opportunityDetail.file')} ${index + 1}`;
                        } else {
                          fileName = attachment.fileName || attachment.s3Key?.split('/').pop() || `${t('sales.opportunityDetail.file')} ${index + 1}`;
                        }
