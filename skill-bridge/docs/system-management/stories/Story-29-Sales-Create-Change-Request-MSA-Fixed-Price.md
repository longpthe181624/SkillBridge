# User Story: Sales Create Change Request for MSA Fixed Price

## Story Information
- **Story ID**: Story-29
- **Title**: Sales Create Change Request for MSA Fixed Price
...
**I want to** create a new change request for a Fixed Price MSA contract through a form  
**So that** I can request modifications to the contract scope, schedule, resources, or other terms, track engaged engineers and billing details, and manage the change request workflow
...
- [ ] I can fill in all required fields for a Fixed Price MSA change request
...
- [ ] When the change request is approved by the client, billing details are automatically added to the MSA contract's billing details (if applicable)
...
- [ ] **Button Location**:
  ...
  - [ ] Button is only visible when viewing a Fixed Price MSA contract

- [ ] **Form Display**:
  - [ ] Form appears as a modal overlay on top of the MSA contract detail page
...
- [ ] **CR Type Field**:
  ...
  - [ ] Field is required (validation required)
  - [ ] Options for Fixed Price MSA contracts:
    - [ ] "Add Scope" is available
    - [ ] "Remove Scope" is available
    - [ ] "Other" is available
  - [ ] Only these three options are shown for Fixed Price MSA contracts
...
- [ ] **Change Request Creation**:
  ...
  - [ ] `contract_id` is set to the current MSA contract ID
  - [ ] `contract_type` is set to "MSA"
...
- [ ] **Billing Details**:
  ...
  - [ ] Billing details are NOT automatically added to MSA contract billing details until approved
...
- [ ] **When Change Request is Approved by Client**:
  ...
  - [ ] Billing details from the change request are automatically added to MSA contract's billing details (if applicable)
  - [ ] Each billing detail row is created with:
    - [ ] `contract_id` set to the MSA contract ID
  ...
  - [ ] MSA contract's total value is updated to include the change request amounts (if applicable)
...
- [ ] **Sales Manager**:
  - [ ] Can create change requests for any Fixed Price MSA contract
  - [ ] Can assign any Sales Manager as internal reviewer
  - [ ] Can save as draft or submit for review

- [ ] **Sales Man (Sales Rep)**:
  - [ ] Can create change requests only for Fixed Price MSA contracts assigned to themselves
  - [ ] Can assign any Sales Manager as internal reviewer
  - [ ] Can save as draft or submit for review
...
- [ ] **API Errors**:
  ...
  - [ ] Handles 404 Not Found (MSA contract not found)
...
- [ ] **POST /api/sales/contracts/msa/{msaContractId}/change-requests**:
  - [ ] Creates a new change request for the specified Fixed Price MSA contract
  - [ ] Accepts multipart/form-data for file uploads
  - [ ] Request body includes:
    - [ ] `title` (required): CR Title
    - [ ] `type` (required): CR Type (Add Scope, Remove Scope, Other)
  ...
  - [ ] Validates that MSA contract is Fixed Price type
  - [ ] Validates user has access to the MSA contract
  ...
  - [ ] Returns 404 Not Found if MSA contract doesn't exist

- [ ] **PUT /api/sales/contracts/msa/{msaContractId}/change-requests/{changeRequestId}/approve** (Future):
  - [ ] Updates change request status to "Approved"
  - [ ] Adds billing details to MSA contract's billing details (if applicable)
  - [ ] Updates MSA contract total value (if applicable)
...
- [ ] **MSA billing details table** (if exists):
...
- [ ] **Integration Tests**:
  ...
  - [ ] Test approval workflow adds billing details to MSA (if applicable)
...
- Overview section: CR Title, CR Type (Add Scope, Remove Scope, Other), Effective from, Effective until
...
- Story-27: Sales List Change Requests for SOW (Change Requests tab structure reference)
- Story-28: Sales Create Change Request for SOW Retainer (implementation pattern reference)
- Story-25: Sales Create MSA (MSA contract detail page must exist)
- Backend API for change requests must be available
- S3 service for file uploads must be configured (optional)

## Notes

- This story focuses on creating change requests for Fixed Price MSA contracts only
- Retainer MSA change requests will be handled in a separate story (if needed)
- The approval workflow that adds billing details to MSA is a key requirement (if applicable)
- Engaged engineers and billing details are specific to Fixed Price MSA change requests
- The form should only be accessible when viewing a Fixed Price MSA contract
- When change request is approved, billing details are automatically integrated into the MSA contract's billing schedule (if applicable)
- CR Type options differ from Retainer: "Add Scope", "Remove Scope", "Other" (instead of "Extend Schedule", "Rate Change", "Increase Resource", "Other")
