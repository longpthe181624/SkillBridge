...
class SalesContractControllerTest {
    ...
    void testGetContracts_SalesManagerSuccess() {
        ...
        when(contractService.getContracts(
                eq("search"), eq("ACTIVE"), eq("MSA"), eq(1), eq(50), eq(user)
        )).thenReturn(serviceRes);

        ResponseEntity<?> response = controller.getContracts(
                "search", "ACTIVE", "MSA", 1, 50, authentication, httpServletRequest
        );

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertSame(serviceRes, response.getBody());
        verify(contractService).getContracts("search", "ACTIVE", "MSA", 1, 50, user);
    }
    ...
    void testGetContracts_RuntimeException() {
        ...
        when(contractService.getContracts(anyString(), anyString(), anyString(), anyInt(), anyInt(), any(User.class)))
                .thenThrow(new RuntimeException("No permission"));

        ResponseEntity<?> response = controller.getContracts(
            "search", "ACTIVE", null, 0, 20, authentication, httpServletRequest
        );
